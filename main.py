...
<tr><td>Context</td><td>{u["ctx"]}</td><td>{d["ctx"]}</td></tr></table>""")
    ...
    def setting(v,side):
     if v["n"]==0: return {"entry":"N/A","tp":"N/A","sl":"N/A","hold":"N/A","ctx":v["ctx"],"status":"NO DATA"}
     entry=("≥ "+num(v["score"])) if side=="up" else ("≤ "+num(v["score"]))
     tp=pct(v["mfe"]); sl=pct(v["mae"]); hold=h
     return {"entry":entry,"tp":tp,"sl":sl,"hold":hold,"ctx":v["ctx"],"status":"EARLY DATA" if v["n"]<1000 else "PAPER TEST"}
