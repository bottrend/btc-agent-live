 ...
 try:
  ...
  if path=="/health": body=json.dumps({"ok":True,**s}).encode(); ct="application/json"
   ...
   for name in GROUPS+["FINAL"]:
    ...
    for h in HORIZONS:
     stat=vals.get(h,{})
     overall=stat.get("all",(None,0)); up=stat.get("up",(None,0)); down=stat.get("down",(None,0))
     ...
     cells+=f'<td>{main}<small>n={overall[1]}</small><small class="dir">↑ {up_txt} ({up[1]}) · ↓ {down_txt} ({down[1]})</small></td>'
