...
@media(max-width:720px){{.wrap{{padding:14px}}.brand{{font-size:25px}}.top .muted{{display:none}}.card{{padding:18px}}.hero{{grid-template-columns:1fr}}.score{{font-size:51px}}.sig{{font-size:31px}}.market{{padding:18px}}.zones{{font-size:10px}}.ticks{{font-size:11px}}.groupsHead h2{{font-size:19px}}.groupsHead .muted{{font-size:12px}}.grow{{grid-template-columns:112px 55px 1fr;gap:8px}}.grow span{{font-size:12px}}.grow b{{font-size:12px}}.grow em{{display:none}}.gtrack{{height:16px}}.summary{{grid-template-columns:1fr 1fr;gap:10px}}.mini{{padding:15px;min-height:125px}}.mini strong{{font-size:20px}}.footer{{font-size:11px}}}}
</style></head><body><div class="wrap"><div class="top"><div><div class="brand">₿ BTC AGENT <span class="green">LIVE</span></div><div class="muted">AI-POWERED · REAL-TIME · {s["features_active"]}/{s["features_total"]} FEATURES</div></div><div class="live">● LIVE<br><small class="muted">Cập nhật tự động</small></div></div>
